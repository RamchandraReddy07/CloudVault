...
export default function FileUpload({ user, onUploadSuccess }: FileUploadProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { uploadUrl } = await api.post<{ uploadUrl: string }>(
        `/api/files/${fileId}/presign-upload`,
        undefined,
        user.token
      )
      setProgress(40)
      ...
      setError(
        err instanceof Error ? err.message : "System Ingestion Failure"
      )
      setIsUploading(false)
      setProgress(0)
    }
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm">
      <h2 className="text-[10px] font-black text-slate-300 mb-8 uppercase tracking-[0.4em]">
        Resource Ingestion
      </h2>

      <div
        className={`relative border-2 border-dashed rounded-[2rem] p-10 transition-all duration-500 flex flex-col items-center justify-center gap-6 group ${
          isUploading
            ? "bg-sky-50/30 border-sky-200"
            : "hover:bg-slate-50/50 hover:border-sky-300 border-slate-100"
        }`}
      >
        <input
          type="file"
        ...
        <div
          className={`w-20 h-20 rounded-3xl flex items-center justify-center transition-all duration-700 shadow-xl ${
            isUploading
              ? "bg-sky-600 text-white scale-110 shadow-sky-100 rotate-6"
              : "bg-white text-slate-300 group-hover:text-sky-600 group-hover:scale-105 shadow-slate-50"
          }`}
        >
          {isUploading ? (
            <svg
        ...
        {isUploading && (
          <div className="w-full max-w-[200px] mt-2">
            ...
            <p className="text-[9px] font-black text-sky-600 mt-3 text-center uppercase tracking-widest">
